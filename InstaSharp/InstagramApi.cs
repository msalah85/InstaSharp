...
namespace InstaSharp
{
    internal class InstagramApi
    {
        ...
        public string PostBase64Image(string base64ImageStr, string userAgent, string imageFormat = "jpeg")
        {
            var url = $"{ApiEndpoint}media/upload/";
            const string paramName = "photo";
            string contentType = string.Format("image/{0}", imageFormat);
            ...
            try
            {
                ...
                using (var requestStream = WebRequest.GetRequestStream())
                {
                    ...
                    var fileName = string.Format("{0}", Guid.NewGuid()); // Path.GetFileName(localImagePath);
                    ...
                    if (!string.IsNullOrEmpty(base64ImageStr))
                    {
                        var imageBytes = Convert.FromBase64String(base64ImageStr);
